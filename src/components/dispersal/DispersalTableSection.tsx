...
interface DispersalTableSectionProps {
...
export const DispersalTableSection: React.FC<DispersalTableSectionProps> = ({
  onTabChange,
  dispersalCount,
  evictionCount
}) => {
  ...
  const getExportDataForScope = (scope: ExportScope, startDate?: string, endDate?: string) => {
    ...
    if (scope === 'custom' && startDate && endDate) {
      return dispersalRecords.filter(d => {
        const val = d.dispersalDate || d.dateReceived || '';
        return (!startDate || val >= startDate) && (!endDate || val <= endDate);
      });
    }
    return dispersalRecords;
  };
  ...
  const renderColumnCell = (col: TableColumnConfig<DispersalRecord>, record: DispersalRecord) => {
    if (col.renderCell) return col.renderCell((record as any)[col.key], record);
    ...
    if (col.key === 'suPortNassRef') return <span className="font-mono text-[#0f766e] font-semibold">{value || '—'}</span>;
    if (col.key === 'flatRoomNumber') return <span className="font-semibold text-[#242424]">{value || '—'}</span>;
    if (col.key === 'sno') return <span className="text-[#605e5c] font-mono">{value || '—'}</span>;
    if (value === null || value === undefined || value === '') return <span className="text-[#a19f9d]">—</span>;
    return String(value);
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-2 border-b border-[#e1dfdd]">
        <div>
          <div className="flex items-center gap-2">
            <PlaneTakeoff className="w-5 h-5 text-[#0078d4]" />
        ...
        <div className="flex items-center gap-2">
          ...
          {canCreateRecord() && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-[#0078d4] hover:bg-[#106ebe] text-white rounded-xs shadow-xs transition-colors cursor-pointer"
            >
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center bg-[#f3f2f1] p-0.5 rounded-xs border border-[#8a8886]/40">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-xs bg-white text-[#0078d4] shadow-xs cursor-pointer"
            >
              <Send className="w-3 h-3 text-[#0078d4]" />
              <span>Dispersal Sheet</span>
              <span className="text-[10px] px-1.5 py-0.2 rounded-full font-bold bg-blue-100 text-[#0078d4]">
                {dispersalCount}
              </span>
            </button>
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              <option value="all">All</option>
              <option value="yes">Has 2nd</option>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {canEditRecord(record.siteName) && (
                          <button
                            ...
                            className="p-1 hover:bg-[#eff6fc] text-[#0078d4] rounded-xs transition-colors cursor-pointer"
